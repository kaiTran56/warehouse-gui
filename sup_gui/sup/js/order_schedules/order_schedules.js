  ...
  dataTable = $("#ordScheTbl").DataTable({
    ...
    columns: [
      ...
      },
      {
        ...
        data: "phone",
        defaultContent: "-",
        className: "text-center txt-truncate",
      },
      {
        title: "Order Code",
        data: "orderCode",
      ...
      {
        title: "Status Order Schedule",
        data: "statusOrderSchedule",
        defaultContent: "-",
        className: " txt-truncate",
      },
      {
      ...
      },
      {
        ...
        render: (data, type, row, meta) => {
          return (
            ...
            '" value="' +
            data +
            '" title="show"  type="button" class="btn bi bi-chat-right-dots  btn-info btn-sm" data-bs-toggle="modal" data-bs-target="#detailOrderSchedule">' +
            "" +
            "</button>" +
            '<button id="done_' +
            data +
            '" value="' +
            data +
            '" title="done" type="button" class="btn btn-success btn-sm bi bi-check2-circle" data-bs-toggle="modal" data-bs-target="#noteOrderSchedule">' +
            "" +
            "</button>" +
            '<button id="deny_' +
            data +
            '" value="' +
            data +
            '" title="deny" type="button" class="btn btn-danger btn-sm bi bi-file-earmark-excel" data-bs-toggle="modal" data-bs-target="#noteOrderSchedule">' +
            "" +
            "</button>" +
            '<button id="note_' +
            data +
            '" value="' +
            data +
            '" title="note" type="button" class="btn btn-warning btn-sm bi bi-pencil-square" data-bs-toggle="modal" data-bs-target="#noteOrderSchedule">' +
            "" +
            "</button>"
          );
        },
...
let saveChangeNote = () => {
  $("#saveNoteOrderSchedule").click((e) => {
    updateStatusObj.id = localStorage.getItem(
      LocalStorageParam.ID_SCHEDULE.name
    );
    updateStatusObj.customerNote = localStorage.getItem(
      LocalStorageParam.CUSTOMER_NOTE.name
    );
    updateStatusObj.productNote = localStorage.getItem(
      LocalStorageParam.PRODUCT_NOTE.name
    );
    updateStatusObj.statusOrderSchedule = localStorage.getItem(
      LocalStorageParam.STATUS_ORDER_SCHEDULE.name
    );
...
let getChangedNote = (status) => {
  ...
  let obj = JSON.parse(
    localStorage.getItem(LocalStorageParam.SCHEDULE_TEMP.name)
  );
...
let showDetailInfo = (id) => {
  $.ajax({
    ...
    success: (response) => {
      ...
      $("#detailOrderScheduleHeader").html(
        "Detail Order: " + response.orderCode
      );
      toastr["success"](SUCCESS_MSG, "SUCCESS");
    },
    error: (err) => {
...
let showNoteOrderSchedule = (id, status) => {
  $.ajax({
    ...
    success: (response) => {
      localStorage.setItem(
        LocalStorageParam.SCHEDULE_TEMP.name,
      ...
      $("#noteOrderScheduleModal").html(
        "Note Order Code: " + response.orderCode
      );

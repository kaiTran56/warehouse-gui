    ...
    dataTable = $('#pickingManagementTbl').DataTable({
        ...
        columnDefs: [
            ...
            },
            {
                ...
                data: 'id',
                render: (data, type, row, meta) => {
                    return '<button id="picked_' + data + '" value="' + data + '" title="picked"  type="button" class="btn bi bi-check2-circle  btn-success btn-sm" >' + '' + '</button>' +
...
let getDetailAction = () => {
    $('#pickingManagementTbl').on('click', 'button', (e) => {
        ...
        if (idBtn.includes(OPERATION_STATUS.PICKED.name)) {
            OrderStatus.status = OPERATION_STATUS.PICKED.value;

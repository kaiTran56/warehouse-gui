    ...
    dataTable = $('#pickingManagementTbl').DataTable({
        ...
        columnDefs: [
            ...
            },
            {
                ...
                data: 'id',
                render: (data, type, row, meta) => {
                    return '<button id="stocktaked_' + data + '" value="' + data + '" title="stocktaked"  type="button" class="btn bi bi-check2-circle  btn-success btn-sm" >' + '' + '</button>' +
...
let getDetailAction = () => {
    $('#pickingManagementTbl').on('click', 'button', (e) => {
        ...
        if (idBtn.includes(OPERATION_STATUS.STOCKTAKED.name)) {
            OrderStatus.status = OPERATION_STATUS.STOCKTAKED.value;
